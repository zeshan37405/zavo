...
function buildProducts(rows) {
  ...
  return rows.slice(1).map((row, index) => ({
    ...
    category: indexes.category >= 0 ? row[indexes.category] : "Tech Accessories",
...
function renderProducts() {
  ...
  grid.innerHTML = visible.map((product) => {
    ...
    const image = product.image || "https://placehold.co/900x675/f1f5f9/334155?text=Smart+Gadget";
    const price = product.price || "See latest price";
    ...
    return `
      <article class="product-card">
        ...
        <div class="product-body">
          <div class="product-category">${escapeHTML(product.category || "Tech Accessories")}</div>
          <h3 class="product-title">${escapeHTML(product.title)}</h3>
          <p class="product-description">${escapeHTML(product.description || "A practical product selected for everyday convenience and value.")}</p>
...
categoryCards.forEach((card) => {
  card.addEventListener("click", (event) => {
    ...
    if (categoryFilter) {
      const matchingOption = [...categoryFilter.options].find((option) => normalize(option.value) === normalize(category));
